...
router.get("/data/:libName/:dataSet",async function(req, res, next) {
    ...
    let isAdditional = req.params.dataSet;
    console.log(dataSetName)
    const parsedData = require("../../src/parsers/parserNeo4j/parseData");
    if(isAdditional) let data = await parsedData.parseNeo4jDataComponent(currentLib,dataSetName)
    else let data = await parsedData.parseNeo4jDataComponent(currentLib,dataSetName)
